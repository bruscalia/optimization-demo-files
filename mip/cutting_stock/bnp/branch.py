...
from bnp.nodes import Node, RootNode


class BranchAndPrice:
    
    def __init__(self, branching_rule="min", node_rule="dfs"):
        """Branch and Price basic algorithm
    ...
    def __call__(
        ...
        pricing_rule : Callable
            Rule that inserts new colunmns into node
        ...
        # Start at root node
        root_node = RootNode(
            c, pricing_rule=pricing_rule, A_ub=A_ub, b_ub=b_ub, A_eq=A_eq, b_eq=b_eq, lb=lb, ub=ub,
            integrality=integrality, branching_rule=self.branching_rule, 
            int_tol=int_tol,
        )
        root_node.solve()
        while root_node.price():
            root_node.solve()
